...
const EditProductClient = ({ product, categories = [], shopId }) => {
  const router = useRouter();
  const api = useApi();

  const handleSubmit = async (values) => {
    const result = await api.patch(`/api/products/${product._id}`, {
  ...
  return (
    <div className="rounded-2xl border border-[#E5E2D8] bg-white p-5 shadow-sm md:p-7">
      <div className="mb-6">
        <h1 className="text-xl font-bold text-[#002B12]">Edit Product</h1>
      ...
      <ProductForm
        ...
        onCancel={() => router.push("/dashboard/seller/products")}
      />
    </div>
  );
};
